...
def MethodOfJoints( input_geometry):
    ...
    for node in nodes:
        # supports load in x dirrection
        if 0 in node.ConstraintType():
            print(f'Reaction in x at node {node.idx} is {node.xforce_reaction}.')
        # supports load in y dirrection
        if 1 in node.ConstraintType():
            print(f'Reaction in y at node {node.idx} is {node.yforce_reaction}.')
